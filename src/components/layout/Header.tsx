...
export function Header() {
  ...
  useEffect(() => { setMobileMenu(null) }, [pathname])

  return (
    <>
      {/* ── Desktop sidebar (hidden on mobile) ───────────────────── */}
      <aside className="hidden md:flex shrink-0 sticky top-0 h-screen z-40">
      ...
      {mobileMenu && (() => {
        const group = GROUPS.find(g => g.id === mobileMenu)!
        return (
          <>
            <div
            ...
            />
            <div className="md:hidden fixed bottom-16 left-0 right-0 z-50 bg-surface border-t border-border shadow-2xl rounded-t-2xl">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-border/60">
                <group.Icon size={16} weight="fill" color="#4a7c59" />
                ...
                {group.items.map(item => {
                  ...
                  return (
                    <button
                      key={item.path}
                      onClick={() => navigate(item.path)}
                      className="flex flex-col items-center gap-2 px-2 py-3 rounded-xl transition-all active:scale-95"
                      style={isActive
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-surface border-t border-border">
        <div className="flex items-stretch h-16">
          {GROUPS.map(group => {
            const isActive = activeGroup.id === group.id
            const isOpen   = mobileMenu === group.id
            return (
              <button
                key={group.id}
                onClick={() => {
                  if (group.items.length === 1) {
                    ...
                    setMobileMenu(isOpen ? null : group.id)
                  }
                }}
                className="flex-1 flex flex-col items-center justify-center gap-1 transition-all active:opacity-60 relative"
              >
                {(isActive || isOpen) && (
                ...
                <group.Icon
                  size={22}
                  weight={isActive ? 'fill' : 'regular'}
                  color={isActive || isOpen ? '#4a7c59' : 'rgb(var(--color-muted))'}
                />
                <span
                  className="text-[10px] leading-none font-medium"
                  style={{ color: isActive || isOpen ? '#4a7c59' : 'rgb(var(--color-muted))' }}
                >
                  {group.label}
                </span>
              </button>
          ...
          <button
            onClick={toggle}
            className="flex-1 flex flex-col items-center justify-center gap-1 text-muted transition-all active:opacity-60"
          >
            {dark ? <Sun size={22} weight="fill" /> : <Moon size={22} weight="fill" />}
            <span className="text-[10px] leading-none font-medium">Tema</span>
          </button>
        </div>
      </nav>
    </>
